(function() {
  ...
  CANVAS_WIDTH =
    CANVAS_WIDTH / CANVAS_HEIGHT > 0.636 ? CANVAS_HEIGHT * 0.636 : CANVAS_WIDTH;
  ...
  var AimBulletBottom =
    CANVAS_HEIGHT - 58 * ratio - AimBullet.h + BULLET_PIERCED_DEEP;
  //调参Json
  /*var Difficulty_Json = {
        "SUCCESS_PARAMETERS" :{
  ...
  if (!window["extendClass"]) {
    window["extendClass"] = function(sonClass, parentClass) {
      if (typeof parentClass === "function" && typeof sonClass === "function") {
        sonClass.prototype = new (function() {}.prototype =
  ...
  CircleCenter.prototype.update = function(
    rs,
    context,
    that,
  ...
  CircleCenter.prototype.resetCircleCenterSplit = function(
    context,
    that,
    centerCircleSplitReposition,
    rs
    ...
    var translatePosition = {
      left: {
        x:
          ROTATION_CENTER.x + centerCircleSplitReposition.left.x - left_w * 0.5,
      ...
      right: {
        x:
          ROTATION_CENTER.x -
          right_cutx +
          centerCircleSplitReposition.right.x +
          right_w * 0.5,
  ...
  Game.prototype = {
    ...
    init: function() {
      ...
      if (!this.centerCircle)
        this.centerCircle = new CircleCenter(
          ballimg,
          Math.PI * 0.5,
          lens,
          ROTATION_CENTER.y,
    ...
    normalResults: function() {
      ...
      if (BulletCirclesIndex == lens) {
        if (arrayCenterCircle[this.levelA + 1]) {
          ...
          this.centerCircle.restNum = this.getParams(
            this.levelA + 1
          ).levelArray[1];
        }
        //插入
        ...
        this.AimBullet = new CircleBullet(
          ROTATION_CENTER.x,
          AimBulletBottom,
          AimBullet.w,
          AimBullet.h,
    ...
    nextLevel: function() {
      ...
      if (self.level <= self.levelLength) {
        ...
        $("#levelSwitchBoxMain").attr(
          "src",
          window.levels_mains[self.level - 1]
        );
    ...
    levelChange: function() {
      ...
      for (var i = 0; i < this.levelLength; i++) {
        document.getElementById("levelbox").children[i].children[0].src =
          levelUrl[i].normal;
        if (i <= this.levelA) {
          document.getElementById("levelbox").children[i].children[0].src =
            levelUrl[i].active;
      ...
      for (i = 0; i < lens; i++) {
        ...
        var ClipTop =
          CANVAS_HEIGHT -
          CLIPSIZE.bottom /*-CLIPSIZE.h*lens*/ -
          CLIPSIZE.space * lens;
        tempClip = new Clip(
          15,
          ClipTop + i * CLIPSIZE.space,
          CLIPSIZE.w,
          CLIPSIZE.h,
          tempClipImg
        );
    ...
    checkCross: function(p1, p2, p3, p4) {
      ...
      if (
        v <= 50000 &&
        this.crossMul(v1, v3) * this.crossMul(v2, v3) <= 50000
      ) {
      }
